...
namespace EnrollmentTester
{
    ...
    public class SmokeTest
    {
        ...
        {
            EnrollUser(number);
            UnenrollUser("0" + number);
        }


        public void EnrollUser(int number)
        {
        ...
        public void UnenrollUser(String username)
        {
            ...
            driver2.Navigate().GoToUrl("https://qatools.orpheusdev.net/");
            WebDriverWait wait = new WebDriverWait(driver2, TimeSpan.FromSeconds(30));
            wait.Until(d => d.FindElement(By.XPath("//select [contains(@id, 'cnnId')]")));
            driver2.FindElement(By.XPath("//select [contains(@id, 'cnnId')]")).Click();
            for (int i = 0; i < 3; i++) action1.SendKeys(Keys.Down).Perform();
            action1.SendKeys(Keys.Enter).Perform();
            driver2.FindElement(By.XPath("//input [contains(@name, 'param')]")).SendKeys(username);
            driver2.FindElement(By.XPath("//button [contains(@id, 'button3')]")).Click();
            try
            {
        ...
        {
            //Confirm
            driver2.FindElement(By.XPath("//input [contains(@class, 'k-textbox size-medium rounded') and contains(@name, 'Questions[0].Answer')]")).SendKeys(question);
            //Verify
